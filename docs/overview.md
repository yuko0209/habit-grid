...
### グリッドの初期表示は `useLayoutEffect` で右端に自動スクロール

グリッド（27週間分）はスマートフォンの画面幅より広いため、初期状態では左端（約半年前）が表示されてしまい、直近の「今日」のセルが画面外に隠れてしまう。
これを防ぐため、コンポーネントのマウント時にコンテナの `scrollLeft` を右端 (`scrollWidth - clientWidth`) に自動スクロールさせている。

- **`useLayoutEffect` の使用:** 画面のチラつきを防ぐため、ブラウザが描画（Paint）する前に同期的にスクロール位置を調整する。
- **日付ロールオーバー時の追従:** `today` の更新（日付変更）を検知して自動スクロールを再度実行する。
- **無駄なスクロールの防止:** 習慣のトグル（セルのタップ）や色の変更など、他のPropsが更新されたときにスクロール位置がリセットされてしまうのを防ぐため、`lastScrolledDateRef` (`useRef`) に最後にスクロールした日付キーを保持。`today` が実質的に変わっていない場合はスクロール処理をスキップしている。
...
| `HabitGrid.test.tsx` | マウント時および日付変更時の右端への自動スクロール、不要なタイミングでのスクロールリセット防止 |
...
1. **記録の一括操作** — 「昨日の分をまとめて付ける」など。旅行明けに欲しくなるはず
2. **週次・月次のサマリー** — 「今週 5/7 達成」のような表示
3. **リマインダー通知** — いまは iOS の「ショートカット」アプリで代用している（[reminder.md](./reminder.md)）。
